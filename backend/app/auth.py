...
async def get_current_user_from_cookie(request: Request, db: AsyncSession = Depends(database.get_db)):
    token = request.cookies.get("access_token")
    if not token:
        # Check Authorization header as fallback (for API clients like curl)
...
# Alias for compatibility if needed, but we should switch routers to use this
