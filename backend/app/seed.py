...
async def seed():
    async with database.AsyncSessionLocal() as db:
        ...
        # Seed User
        seed_username = os.getenv("SEED_USERNAME", "ariel")
        seed_password = os.getenv("SEED_PASSWORD")

        if not seed_password:
            print("Skipping user seed: SEED_PASSWORD is not set")
        else:
            result = await db.execute(select(models.User).where(models.User.username == seed_username))
            ...
            if not user:
                ...
                print(f"User exists: {seed_username} - Updating password to match current scheme")
